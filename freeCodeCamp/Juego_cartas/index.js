...
function gameOver() {
  ...
  const gameOverMessage = `¡Fin de Juego! Puntuación - <span class = 'badge'>${score}</span> Presiona 'Play Game' Jugar nuevamente`;
  ...
  if (canChooseCard()) {
    ...
    setTimeout(() => {
      ...
      updateStatusElement(
        ...
        "block",
        primaryColor,
        "Posiciones de cartas reveladas"
      );
...
function updateScore() {
  ...
  updateStatusElement(
    ...
    "block",
    primaryColor,
    `Puntaje <span class='badge'>${score}</span>`
  );
}
...
function checkForIncompleteGame() {
  ...
  if (serializedGameObj) {
    ...
    if (gameObj.round >= maxRounds) {
      ...
      if (confirm("Te gustaría continuar con tu último juego?")) {
        score = gameObj.score;
...
function initializeNewGame() {
  ...
  updateStatusElement(
    ...
    "block",
    primaryColor,
    `Ronda <span class='badge'>${roundNum}</span>`
  );
}
function startRound() {
  initializeNewRound();
  ...
  function shuffle() {
    ...
    if (shuffleCount == 500) {
      ...
      updateStatusElement(
        ...
        "block",
        primaryColor,
        "Haz clic en la carta que creas que es el as de picas......"
      );
    } else {
      shuffleCount++;
    }
